...
- if this is unsuccessful (or no previous network saved) it moves the ESP into Access Point mode and slogger up a DNS and WebServer (default ip 192.168.4.1)
...
__Currently version 0.8 works with release 2.0.0 or newer of the [ESP8266 core for Arduino](https://github.com/esp8266/Arduino)__
 ...
 - search for logger
  ![logger package](http://i.imgur.com/18yIai8.png)
...
logger.autoConnect("AP-NAME", "AP-PASSWORD");
```
if you just want an unsecured access point
```cpp
logger.autoConnect("AP-NAME");
```
...
Also see [examples](https://github.com/tzapu/logger/tree/master/examples).
...
```cpp
logger.autoConnect("AutoConnectAP", "password")
```
...
Before `autoConnect()`
```cpp
logger.setAPCallback(configModeCallback);
```
`configModeCallback` declaration and example
```cpp
void configModeCallback (logger *mylogger) {
  Serial.println("Entered config mode");
  Serial.println(WiFi.softAPIP());

  Serial.println(mylogger->getConfigPortalSSID());
}
```
...
See [AutoConnectWithFSParameters Example](https://github.com/tzapu/logger/tree/master/examples/AutoConnectWithFSParameters).
```cpp
logger.setSaveConfigCallback(saveConfigCallback);
```
...
```cpp
logger.setConfigPortalTimeout(180);
```
...
void loop() {
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    logger logger;
    logger.startConfigPortal("OnDemandAP");
    Serial.println("connected...yeey :)");
  }
}
```
See example for a more complex version. [OnDemandConfigPortal](https://github.com/tzapu/logger/tree/master/examples/OnDemandConfigPortal)

### Custom Parameters
You can use logger to collect more parameters than just SSID and password.
...
- add the custom parameters to logger using
 ```cpp
 // id/name, placeholder/prompt, default, length
 loggerParameter custom_mqtt_server("server", "mqtt server", mqtt_server, 40);
 logger.addParameter(&custom_mqtt_server);

 ```
- if connection to AP fails, configuration portal starts and you can set /change the values (or use on demand configuration portal)
- once configuration is done and connection is established [save config callback]() is called
- once logger returns control to your application, read and save the new values using the `loggerParameter` object.
 ```cpp
 mqtt_server = custom_mqtt_server.getValue();
 ```  
This feature is a lot more involved than all the others, so here are some examples to fully show how it is done
- Save and load custom parameters to file system in json form [AutoConnectWithFSParameters](https://github.com/tzapu/logger/tree/master/examples/AutoConnectWithFSParameters)
...
logger.setAPStaticIPConfig(IPAddress(10,0,1,1), IPAddress(10,0,1,1), IPAddress(255,255,255,0));
```

### Custom Station (client) Static IP Configuration
This will make use the specified IP configuration instead of using DHCP in station mode.
```cpp
logger.setSTAStaticIPConfig(IPAddress(192,168,0,99), IPAddress(192,168,0,1), IPAddress(255,255,255,0));
```
...
#### Filter Networks
If you would like to filter low signal quality networks you can tell logger to not show networks below an arbitrary quality %;
```cpp
logger.setMinimumSignalQuality(10);
```
