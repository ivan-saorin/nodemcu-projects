...
__Currently version 0.8 works with release 2.0.0 or newer of the [ESP8266 core for Arduino](https://github.com/esp8266/Arduino)__
 ...
 - search for pins
  ![pins package](http://i.imgur.com/18yIai8.png)
...
pins.autoConnect("AP-NAME", "AP-PASSWORD");
```
if you just want an unsecured access point
```cpp
pins.autoConnect("AP-NAME");
```
...
Also see [examples](https://github.com/tzapu/pins/tree/master/examples).
...
```cpp
pins.autoConnect("AutoConnectAP", "password")
```
...
Before `autoConnect()`
```cpp
pins.setAPCallback(configModeCallback);
```
`configModeCallback` declaration and example
```cpp
void configModeCallback (pins *mypins) {
  Serial.println("Entered config mode");
  Serial.println(WiFi.softAPIP());

  Serial.println(mypins->getConfigPortalSSID());
}
```
...
See [AutoConnectWithFSParameters Example](https://github.com/tzapu/pins/tree/master/examples/AutoConnectWithFSParameters).
```cpp
pins.setSaveConfigCallback(saveConfigCallback);
```
...
```cpp
pins.setConfigPortalTimeout(180);
```
...
void loop() {
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    pins pins;
    pins.startConfigPortal("OnDemandAP");
    Serial.println("connected...yeey :)");
  }
}
```
See example for a more complex version. [OnDemandConfigPortal](https://github.com/tzapu/pins/tree/master/examples/OnDemandConfigPortal)

### Custom Parameters
You can use pins to collect more parameters than just SSID and password.
...
- add the custom parameters to pins using
 ```cpp
 // id/name, placeholder/prompt, default, length
 pinsParameter custom_mqtt_server("server", "mqtt server", mqtt_server, 40);
 pins.addParameter(&custom_mqtt_server);

 ```
- if connection to AP fails, configuration portal starts and you can set /change the values (or use on demand configuration portal)
- once configuration is done and connection is established [save config callback]() is called
- once pins returns control to your application, read and save the new values using the `pinsParameter` object.
 ```cpp
 mqtt_server = custom_mqtt_server.getValue();
 ```  
This feature is a lot more involved than all the others, so here are some examples to fully show how it is done
- Save and load custom parameters to file system in json form [AutoConnectWithFSParameters](https://github.com/tzapu/pins/tree/master/examples/AutoConnectWithFSParameters)
...
pins.setAPStaticIPConfig(IPAddress(10,0,1,1), IPAddress(10,0,1,1), IPAddress(255,255,255,0));
```

### Custom Station (client) Static IP Configuration
This will make use the specified IP configuration instead of using DHCP in station mode.
```cpp
pins.setSTAStaticIPConfig(IPAddress(192,168,0,99), IPAddress(192,168,0,1), IPAddress(255,255,255,0));
```
...
#### Filter Networks
If you would like to filter low signal quality networks you can tell pins to not show networks below an arbitrary quality %;
```cpp
pins.setMinimumSignalQuality(10);
```
